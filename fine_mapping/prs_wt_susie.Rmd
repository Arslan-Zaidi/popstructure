...
Calculate the bias ($\rho(PS,latitude)$) and prediction accuracy ($\rho^2(PS,genetic~value)$) for each replicate. Plot histograms of these. 

```{r}

prs.summary = prs.both%>%
...
plt.bias = ggplot(prs.summary)+
  ...
  geom_vline(data = prs.summary.mean, 
             aes(xintercept = mean.rlat),
             linetype = "dashed",
             color = "red")


plt.r2 = ggplot(prs.summary)+
...
prs.summary = rbind(prs.summary, mprs.lead.summary)
...
plt.susie = (plt.bias + coord_cartesian(xlim = xrange.rlat)) + (plt.r2 + coord_cartesian(xlim = xrange.r2))

plt.lead = plt.lead.bias + plt.lead.r2

plt.all = plt.susie / plt.lead
