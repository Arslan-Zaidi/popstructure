...
fsummarize = function(phenotype = "smooth",
                      correction = "maf1"){
  #read the dataframe
  dat = fread(F(paste("gwas/ukb/train/gwas_results/fixed_effects/noge/revisions/gwas_ukb_train.noge.1.uniform.",
...
plt1 = ggplot(data = dat.both)+
  ...
  labs(y = bquote(lambda[median]),
       title = "A) Genomic inflation as a function\n of different sets of PCs",
...
cm = fread(F("gwas/ukb/train/genotypes/revisions/genos_ukb_l1e7_ss500_m0.08_uniform_chr1_20.rmdup.train.cm.200k.pca.eigenvec.gz"))

re = fread(F("gwas/ukb/train/genotypes/revisions/genos_ukb_l1e7_ss500_m0.08_uniform_chr1_20.rmdup.train.re.1M.pca.eigenvec.gz"))

ibd = fread(F("gwas/ukb/train/germline/germ_ukb_ibd_all.cm.pca.eigenvec"))

colnames(cm) = colnames(re) = colnames(ibd) =
  c("FID","IID",paste("PC",seq(1,100),sep=""))

#read info of which deme each individual comes from
pop = fread(F("gwas/ukb/popfiles/ukb_ss500_d35.uniform.pop"))
...
plt.cm = ggplot(cm, aes(PC1,PC2,
  ...
  labs(title = "B) PCA on common variants \n (MAF > 0.05)")
...
plt.re = ggplot(re, aes(PC1,PC2,
  ...
  theme(legend.position="none",
        ...
        plot.background = element_blank(),
        plot.title = element_text(hjust = 0) )+
  labs(title = "C) PCA on rare variants \n (MAC: 2-4)")
...
plt.combined = (plt1 + plt.cm) / (plt.re + plt.ibd) 

plt.combined

ggsave(F("plots/PCA_UK.png"),
       plt.combined,
       height=7, 
       width = 7)
```
