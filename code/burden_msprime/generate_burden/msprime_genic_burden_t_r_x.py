...
req_grp.add_argument("--cm_pca","-a",dest="cm",help="common PCA file",type=str,required=True)
req_grp.add_argument("--re_pca","-b",dest="re",help="rare PCA file",type=str,required=True)
req_grp.add_argument("--tmove","-t",dest="tmove",help="time (g) to panmixia. can be -9 (inf) or any positive integer",type=int,required=True)
...
if args.rho == 0:
    ...
    burden = np.empty((ss*args.ndemes,100))
...
np.savetxt("output/association/t"+str(args.tmove)+"/r"+str(args.rho)+"/pburden_x"+str(args.seed)+"_r"+str(args.rho)+".txt",np_p1,delimiter=",",fmt='%6.6g')
