...
req_grp.add_argument("--legend", "-l", dest="legend", help="snp file corresponding to row order in ht array", type=str, required=True)
req_grp.add_argument("--npairs", "-n", dest="npairs", help="number of sibs to keep", type=int, required=True)
req_grp.add_argument("--sample","-s", dest="samples", help="sample file corresponding to column order in ht array", type=str, required=True)
...
#keep npairs siblings
#that's npairs*2 individuals and npairs*2*2 haplotypes
ht=ht[:,0:(args.npairs*4)]

#read legend file - snps info
snps = pd.read_csv(args.legend,
                   delim_whitespace=True)

#read sample file - individual info
samples = pd.read_csv(args.samples,
                        delim_whitespace=True)

#keep npairs siblings
samples = samples.iloc[0:(args.npairs*2),:]
...
#keep phenotypes only for individuals in the sample file
phenotypes = phenotypes[(phenotypes['FID'].isin(samples.fid)) & (phenotypes['IID'].isin(samples.iid))]
