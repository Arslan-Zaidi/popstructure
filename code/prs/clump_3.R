...
fcausal_p = function(df,pvalue=5e-04){
    ...
    df=df%>%
    mutate(P = case_when(P < pvalue ~ 0,
                          TRUE ~ P))
    return(df)
}

gwas1 = fcausal_p( gwas1.1 )
gwas2 = fcausal_p( gwas2.1 )
gwas3 = fcausal_p( gwas3.1 )

gwas.causal.p = cbind( gwas1.1[,c("ID","A1","BETA1")],
                    gwas2.1[,c("BETA2")],
                    gwas3.1[,c("BETA3")])
...
gwas.red = merge(gwas1.red, gwas2.red[,c("ID","BETA2")], by="ID", all=TRUE)
gwas.red = merge(gwas.red, gwas3.red[,c("ID","BETA3")] , by="ID", all=TRUE)
gwas.red$A1="T"

gwas.red = gwas.red%>%
            replace_na(list( BETA1=0 , BETA2 = 0, BETA3 = 0))
